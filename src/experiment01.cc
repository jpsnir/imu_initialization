...
void run(const fs::path &sequence_path)
{
  ...
  LOG(INFO) << "delta time between gt trajectory " << trajectory[n - 1].timestamp - trajectory[0].timestamp;
  io::ImuData imu_data = io::read_file<io::ImuData::value_type>(data_path.string());
  int n1 = imu_data.size();
  LOG(INFO) << "imu_data  start :" << imu_data[0].timestamp << " gt - imu start " << trajectory[0].timestamp - imu_data[0].timestamp;
  LOG(INFO) << "imu_data  end :" << imu_data[n1 - 1].timestamp << " gt - imu end " << trajectory[n - 1].timestamp - imu_data[n1 - 1].timestamp;
  ...
  for (unsigned nframes : possible_nframes)
  {
    ...
    // build the dataset for defining initialization problem for the complete trajectory.
    // Here we are trying to extract segments of data to solve the initialization problem
    // the input_t struct defines the data input. 
    while (i != trajectory.cend())
    {
      InputType input;
      ...
      for (unsigned n = 0; n < nframes; ++n)
      {
        ...
        LOG(INFO) << "Imu time stamp begin: " << it->timestamp << " gt timestamp : " << i->timestamp << " diff : " << i->timestamp - it->timestamp;

        Groundtruth::const_iterator j = next(i, trajectory.cend(), 250000000); // 4 Hz
        LOG_EVERY_N(INFO, 10) << "ground truth trajectory timestamp : " << j->timestamp;
